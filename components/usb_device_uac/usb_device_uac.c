...
/* ============ 本地修改 #2 (相对 espressif/usb_device_uac 0.2.0) ============
 *
 * 上游从来没有处理过麦克风 Feature Unit 的控制请求，但描述符里又把它的
 * 静音/音量声明成了「主机可编程」(MIC_CTRL = 0x0F)。于是：
 *
 *   Windows 向实体 0x12 (UAC2_ENTITY_MIC_FEATURE_TERMINAL) 查音量
 *     -> tud_audio_get_req_entity_cb 只认 0x04 和 0x02，掉进 else
 *     -> 返回 false，TinyUSB STALL 掉这个控制请求
 *     -> usbaudio2.sys 启动失败，CM_PROB_FAILED_START (代码 10)
 *
 * 现象和描述符非法时一模一样：设备管理器里一个带感叹号的「usb uac」，
 * 录音设备列表里什么都不出现。但这次描述符是完全合规的，问题在控制端点上。
 *
 * 顺带修掉一个越界：mute/volume 数组原来按 N_CHANNELS_TX（扬声器声道数）定长，
 * 我们把扬声器关掉之后它只有 1 个元素，而麦克风有 master+2 共 3 个通道，
 * 按 bChannelNumber 索引会读写到结构体后面的字段上去。
 */
#define UAC_MAX_FU_CHANNELS      ((CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_TX > CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX)      ? CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_TX : CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX)

typedef struct {
    usb_phy_handle_t phy_hdl;
    uac_device_config_t user_cfg;
    int8_t mute[UAC_MAX_FU_CHANNELS + 1];                        // +1 for master channel 0
    int16_t volume[UAC_MAX_FU_CHANNELS + 1];                     // +1 for master channel 0
...
{
    /* 本地修改 #2：麦克风的 Feature Unit 也要认，否则 Windows 查不到就 STALL */
    TU_ASSERT(request->bEntityID == UAC2_ENTITY_SPK_FEATURE_UNIT ||
              request->bEntityID == UAC2_ENTITY_MIC_FEATURE_TERMINAL);
    TU_VERIFY(request->bChannelNumber <= UAC_MAX_FU_CHANNELS);
...
bool tud_audio_get_req_entity_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (request->bEntityID == UAC2_ENTITY_SPK_FEATURE_UNIT ||
        request->bEntityID == UAC2_ENTITY_MIC_FEATURE_TERMINAL) {   /* 本地修改 #2 */
